...
##            print(infinite[:-1])
##    else:pass
def Decrypt(converted_string,n,key):
    """n is layers of encryption max is 3
    key can be list(n=3) or str(n= 1 or 2)"""
    if type(key)==list and n==3:
        BifidCipher.Bifid_Rev(converted_string,key)
        ...
    elif type(key)==str and n==1 and len(key)==9:
        ...
    elif type(key)==str and n==2 and len(key)==10:
        ...
    else:
        raise TypeError("Key does not match with encryption level")
